document.addEventListener('DOMContentLoaded', () => {
    ...
    // CORRECCIÓN ALTERNATIVA: Se cambiaron los valores a minúsculas
    const ApiAction = {
    ...
    async function callWebApp(action, payload) {
        ...
        try {
            // El cuerpo se envía como texto sin formato (text/plain)
            const response = await fetch(GOOGLE_WEB_APP_URL, {
                method: 'POST',
                mode: 'cors',
                credentials: 'omit',
                // No se especifica 'Content-Type' para que la petición sea una "solicitud simple" de CORS
    ...
    function renderResultsView() {
        ...
        const totalCost = ingredients.reduce((acc, ing) => acc + (ing.estimatedLocalPrice || 0), 0);
        ...
        const recipeDetailsHTML = `
            <div class="mb-6">
                <h2 class="text-xl font-bold text-gray-700 mb-2">🛒 Ingredientes</h2>
                <ul class="space-y-2">${ingredients.map(ing => `
                    <li class="grid grid-cols-[1fr_auto] items-start gap-4 bg-white p-3 rounded-lg border shadow-sm">
                        <div>
                        ...
                        <div class="bg-gray-100 p-2 rounded-md text-center w-[80px] flex-shrink-0">
                            <p class="text-sm font-bold text-gray-800">${ing.estimatedLocalPrice > 0 ? ing.estimatedLocalPrice.toFixed(2) : '-'}</p>
    ...
    function handleShare(platform, totalCost) {
        const generateText = (isWhatsapp) => {
            ...
            state.recipe.ingredients.forEach(ing => {
                txt += `- ${ing.quantity.toFixed(2).replace(/\.00$/, '')} ${ing.unit} de ${ing.name} (aprox. ${(ing.estimatedLocalPrice || 0).toFixed(2)} ${state.recipe.currencyCode})${nl}`;
            });
            txt += `${nl}Costo Total Estimado: ${totalCost.toFixed(2)} ${state.recipe.currencyCode}${nl}`;
