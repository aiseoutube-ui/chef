// CusiCusa - Script Definitivo (vFinal)
...
// LÓGICA DE PORCIONES: Referencia al selector de porciones
...
let currentRecipeForDisplay = null; // Para la receta escalada por porciones
...
let currentServings = 1; // LÓGICA DE PORCIONES: Estado inicial
...
function launchConfetti() {
    ...
    for (let i = 0; i < 150; i++) {
        ...
        confetti.style.animation = `fall ${Math.random()*3+4}s linear ${Math.random()*2}s forwards`;
...
function showPremiumModal(isCooldown) {
    ...
    if (isCooldown) {
        ...
        modalCooldownInfo.innerHTML = "Tu límite de análisis ha sido alcanzado.";
        if (countdownInterval) clearInterval(countdownInterval);
    }
...
window.onload = async function() {
    ...
    // LÓGICA DE PORCIONES: Event listener para el cambio de porciones
    if (servingsSelector) {
...
function updateDisplayAndControls() {
    ...
    modalAdCounter.textContent = `${adCount}/${adBonusLimit}`;
    const isCooldownActive = lastAnalysisTimestamp !== 0 && (Date.now() - lastAnalysisTimestamp) < COOLDOWN_MS && !isBonusActive;
    if (isCooldownActive) {
    ...
    const canGetBonus = (adBonusLimit - adCount) > 0;
    bonusButton.classList.toggle('hidden', !((isCooldownActive || !hasUses) && canGetBonus));
}
...
function simulateAdView() {
    ...
    // SOLUCIÓN: Esta es la llamada correcta a la app de Android.
    if (window.Android && typeof window.Android.showRewardedAd === 'function') {
        window.Android.showRewardedAd();
    } else {
        console.log("Simulando ad reward porque la interfaz Android no está disponible.");
        // Fallback para pruebas en navegador: simular la recompensa después de 2s
...
async function grantAdBonusFromAndroid() {
    ...
    viewAdButton.querySelector('#ad-button-text').innerHTML = `Usar Bono (<span id="modal-ad-counter">${usageStatus.adCount}/${usageStatus.adBonusLimit}</span> disp.)`;
...
async function claimAdBonus() {
    ...
    } catch (e) { console.error("Error claiming bonus:", e); }
}
...
async function analyzeImage(base64ImageData) {
    ...
    try {
        ...
        if (result.success) {
            ...
            usageStatus = result.status;
            // LÓGICA DE PORCIONES: Mostrar para las porciones actuales
            updateDisplayForServings(); 
        } else {
            usageStatus = result.status;
            updateDisplayAndControls();
...
function handleFileSelect(e) { if (e.target.files[0]) { const r = new FileReader(); r.onload = e => handleImageData(e.target.result); r.readAsDataURL(e.target.files[0]); } }
...
function closeCamera() { if (mediaStream) mediaStream.getTracks().forEach(t => t.stop()); cameraModal.classList.add('hidden'); }
...
function switchCamera() { currentFacingMode = currentFacingMode === 'environment' ? 'user' : 'environment'; openCamera(); }

// --- Lógica de Resultados ---

// LÓGICA DE PORCIONES: Nueva función para escalar la receta
function updateDisplayForServings() {
    ...
    scaledRecipe.ingredients.forEach(ing => {
        ing.quantity = (parseFloat(ing.quantity) || 0) * currentServings;
...
function displayResults(data) {
    ...
    dishNameEl.textContent = `${data.dishName || "Plato no identificado"} (para ${currentServings} ${currentServings > 1 ? 'p.' : 'p.'})`;
    ...
    data.ingredients.forEach(ing => {
        ...
        li.innerHTML = `
            <div class="flex items-start">
                ...
                <div><span class="font-semibold text-gray-800">${ing.name}</span><span class="block text-xs text-gray-500">${quantity.toFixed(2).replace(/\.00$/, '')} ${ing.unit}</span></div>
            </div>
    ...
    baseRecipeForOne.instructions.forEach(step => { instructionListEl.innerHTML += `<li class="pl-2 leading-relaxed">${step}</li>`; });
    
    supermarketSectionEl.classList.toggle('hidden', !data.supermarketSuggestions?.length);
    if(data.supermarketSuggestions?.length) data.supermarketSuggestions.forEach(store => supermarketListEl.innerHTML += `<span class="bg-green-100 text-green-800 text-sm font-semibold px-3 py-1.5 rounded-full">${store}</span>`);
...
function resetUI() {
    ...
    uploadOptionsContainer.classList.remove('hidden');
    imagePreviewContainer.classList.add('hidden');
    // LÓGICA DE PORCIONES: Resetear el selector
    if (servingsSelector) document.getElementById('servings1').checked = true;
    currentServings = 1;
    updateDisplayAndControls();
}
...
function generateShareableText(platform = 'clipboard') {
    if (!currentRecipeForDisplay) return ""; // LÓGICA DE PORCIONES: Usar la receta escalada
    const recipe = currentRecipeForDisplay;
    const totalCost = recipe.ingredients.reduce((acc, ing) => acc + (ing.estimatedLocalPrice || 0), 0);
    const title = text => platform === 'whatsapp' ? `*${text}*` : text.toUpperCase();
    let shareText = `${title(recipe.dishName)}\n\n🛒 ${title("Lista de Compras")}\n`;
    recipe.ingredients.forEach(ing => {
        shareText += `- ${(parseFloat(ing.quantity) || 0).toFixed(2).replace(/\.00$/, '')} ${ing.unit} de ${ing.name} (aprox. ${(ing.estimatedLocalPrice || 0).toFixed(2)} ${recipe.currencyCode})\n`;
    });
    shareText += `\nCosto Total Estimado: ${totalCost.toFixed(2)} ${recipe.currencyCode}\n`;
    if (recipe.supermarketSuggestions?.length) shareText += `\n🏪 ${title("Puedes comprar en")}\n${recipe.supermarketSuggestions.join(', ')}\n`;
    shareText += `\n📝 ${title("Instrucciones")}\n`;
    // LÓGICA DE PORCIONES: Usar las instrucciones de la receta base (no se escalan)
...
function share(platform) { if (platform === 'whatsapp') window.open(`https://wa.me/?text=${encodeURIComponent(generateShareableText(platform))}`, '_blank'); }

// --- Lógica de la intro y ofertas ---
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => document.getElementById('intro-overlay').classList.add('opacity-0', 'pointer-events-none'), 3000);
});
const offers = [
    { image: "https://images.unsplash.com/photo-1518843875459-f738682238a6?q=80&w=2070&auto=format&fit=crop", title: "¡Conviértete en Chef Pro!", description: "Análisis ilimitados y funciones exclusivas.", buttonText: "Ver Planes", buttonClass: "bg-teal-500 hover:bg-teal-400" },
    ...
    { image: "https://images.unsplash.com/photo-1498837167922-ddd27525d352?q=80&w=2070&auto=format&fit=crop", title: "¿Dieta especial?", description: "Desbloquea recetas keto, veganas, y más.", buttonText: "Explorar Dietas", buttonClass: "bg-green-500 hover:bg-green-400" }
];
...
function startOfferRotation() { updateOfferCard(); setInterval(() => { currentOfferIndex = (currentOfferIndex + 1) % offers.length; updateOfferCard(); }, 5000); }

// --- Lógica para Animaciones de Carga ---
const LOADING_MESSAGES = [ "Analizando sabores...", "Consultando a chefs de IA...", "Calculando costos locales...", "¡Casi listo! Toques finales..." ];
